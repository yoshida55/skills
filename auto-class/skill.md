...
> ## 🖱️ 適用範囲（絶対厳守）
> - ユーザーがドラッグ（選択）した箇所だけに適用する
> - `<ide_selection>` タグ内のコードが対象
> - 選択範囲がない場合は「対象コードを選択してください」と伝えてスキルを終了する
> - ファイル全体を勝手に読み込んで全修正するのは禁止
...
> **クラス名は2単語が絶対原則。3単語を作りそうになったら必ず止まって考え直す。**

### 3単語になりそうなときの思考フロー（必須）
```
3単語になりそう → まず下記を確認する

① CSS の親セレクタで代用できないか？
   例: .footer_contact a { } で書けるなら <a> にクラス不要

② 役割を直接表す2単語にできないか？
   例: footer_store_link → footer_online（役割を直接表現）

③ ①②どちらも無理な場合のみ → 3単語を使う（最終手段）
```

### 装飾・区切り線はHTMLに書かない（絶対厳守）
- `-` `|` などの装飾文字 → `::before` / `::after` で CSS に書く
- HTMLに `<div>-</div>` を書くのは**禁止**

---

### 基本ルール
```
原則: [section]_[element]          (2単語・これが基本・常にここから考える)
最終手段: [section]_[element]_[detail] (3単語・上記フローを経ても無理な場合のみ)

例：
✅ header_title    （2単語 - 基本形）
✅ footer_online   （2単語 - 役割を直接表現）
✅ footer_contact a（CSSセレクタで代用 - クラス不要）
⚠ header_item_title（3単語 - 2単語で無理な場合のみ）
❌ header_item_title_active（4単語 - 絶対禁止）
```

### 使い分け
- **2単語が原則**: まず2単語で命名できないか考える。できるなら必ず2単語にする
- **3単語の前にCSSで代用できないか確認する**: `.親クラス 子タグ` で書けるなら個別クラス不要
- **3単語は最終手段**: 上記を経ても無理な場合のみ
- **4単語以上禁止**: 構造を見直すか命名を変える
...
```
TodoWrite([
  { content: "デザインカンプの提出をユーザーに依頼",               status: "in_progress", activeForm: "デザインカンプを確認中" },
  { content: "既存HTMLをgrepして共通クラス確認",                   status: "pending",     activeForm: "既存クラスをgrep確認中" },
  ...
  { content: "全タグにクラス名付与（インラインテキストも含む）",    status: "pending",     activeForm: "クラス名付与中" },
...
> ### 🖼 デザインカンプの要求（Step 0 で必ず実行）
> TodoWrite を出した直後、以下の文言でユーザーにデザインカンプを求める：
> **「デザインカンプ（完成イメージの画像）を貼ってください。セクション名・クラス名の判断に使います。」**
> - カンプがあれば → カンプを正式な情報源として使う
> - カンプがなければ → HTMLのコメントとテキスト内容からクラス名を判断する

- ⚠ **スクショが提供された場合、スクショが正式な情報源**
- ⚠ **読み取れない箇所は推測せず、必ずユーザーに質問する**
- ⚠ **共通クラスが見つかったら、新規クラスを作らず使い回す**

### Step 1. ユーザーがHTMLタグのみ記述（インラインテキストも含む）
```html
<div>
  <h2>質問タイトル</h2>
  <p>回答内容</p>
...
### Step 2. AIが全タグ＋インラインテキストにクラス名付与
- `<span>` `<strong>` `<em>` `<a>` などインライン要素にも必ずクラスを付ける
- テキストの役割（強調・注釈・ラベルなど）からクラス名を判断する
...
<div class="accordion_area">
  <div class="accordion_item">
    ...
    <span class="accordion_note">補足テキスト</span>
  </div>
</div>
```
...
3. **2単語が原則**: まず2単語で命名を試みる。3単語は2単語では区別できない場合の最終手段
