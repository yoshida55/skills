...
### Step 0. 事前チェックリスト（クラス付与の前に必ず実行）

以下を画面に表示し、1つずつ確認してから作業に入る:

```
📋 auto-class 事前チェック
□ 既存HTMLをgrepして、同じデザインの共通クラスがないか確認した？
□ スクショの文言が全部読み取れた？（読めない箇所はユーザーに確認）
□ スクショと既存テキストに差分がないか確認した？（文言・電話番号・リンクテキスト等）
```
