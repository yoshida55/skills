...
Step4.6: 書く内容 + AI提案をチャットに出す（← 必ず実施）
         → 以下の形式でユーザーに見せてから memo_write_all を実行する

         ✅ 書く：トピック名（新規 or 既存追記）
         💡 これも書けます：トピック名
            （今日の会話で出てきた・ユーザーが明示しなかったが汎用的な知識としてメモ価値あり）

         → 確認待ちはしない・✅ の分だけそのまま書き込む
         → 💡 はユーザーへの提案のみ。「書いて」と言われたら追加する
