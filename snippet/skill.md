...
- **一番使えるものを先頭に配置する**
- **コードに補足説明を入れる場合は、コード直下に1行メモを添える**
  - 例：`// 40%見えたら発動` など、数値や判定条件の意味が伝わるように
- **複数バージョンのコードがある場合（jQueryとVanilla JSなど）はユーザーに確認して効率のよい方を採用する**
  - 「どちらを残しますか？」と聞く
  - 採用しなかった方は比較表としてまとめて末尾に残す
- **比較表を書く場合は、表の前後に文章の説明も添える**
  - 表だけだと「なぜ違うのか」が伝わらない
  - 「旧版は〇〇が重い理由」「新版でどう改善されたか」を言葉で書く
- **会話中にユーザーが疑問に思ったこと（「〇〇はミス？」など）は補足セクションにQ&A形式で記録する**
  - ファイル末尾の `## 補足` セクションに追加
  - 例：`### Q. JSが2つ書いてあるけどミス？ → 片方は説明用・片方はコピペ用の意図。不要なら削除してOK`
