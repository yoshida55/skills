...
```
TodoWrite([
  { content: "PC判定（自宅 or 会社）※1会話1回だけ",            status: "in_progress" },
  { content: "ユーザーに出力ファイル名・提出先フォルダを確認",   status: "pending" },
...
### PC判定（1会話に1回だけ実行・2回目以降はスキップ）

```bash
test -f "D:/50_knowledge/00_mis_log.md" && echo "home" || echo "company"
```

| 結果 | PC | last_config.json のパス |
|------|----|------------------------|
| home | 自宅PC | `C:\Users\sensh\.claude\skills\deliver\last_config.json` |
| company | 会社PC | `C:\Users\guest04\.claude\skills\deliver\last_config.json` |

判定結果を変数として覚えておき、**同じ会話内では2回目以降は判定をスキップする。**

---

### 前回の提出先を確認する

上記で確定したパスの `last_config.json` を Read で読み込み、前回の提出先を取得する。
